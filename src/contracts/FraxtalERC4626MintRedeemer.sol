...
contract FraxtalERC4626MintRedeemer is OwnedV2AutoMsgSender, ReentrancyGuard {
    ...
    /// @notice Last block.number the vault token oracle was read
    uint256 public lastVaultTknOracleRead;

    /// @notice Micro Fee for deposit|mint/withdraw|redeem flow. 18 decimals
    uint256 public fee;

    /// @notice Latest underlying token price read from the oracle
    uint256 public underlyingTknPrice;

    /// @notice Last block.number the underlying token oracle was read
    uint256 public lastUnderlyingTknOracleRead;
    ...
    function initialize(
        ...
        if (IERC20Metadata(_underlyingTkn).decimals() != 18) revert UnderlyingTokenMustBe18Decimals();
        ...
        if (priceFeedVault.decimals() != 18) {
            revert FeedsMustBe18Decimals();
        }
        if (_underlyingOracle != address(0) && priceFeedUnderlying.decimals() != 18) {
            revert FeedsMustBe18Decimals();
        }
        // Set initial vault token price
        updateVaultAndUnderlyingOracles();
    ...
    /// @notice Get the stored underlying token price
    function getUnderlyingTknPriceStoredE18() public view returns (uint256 _price) {
        // Return the last oracle price ingested
        return underlyingTknPrice;
    }
    ...
    function _convertToShares(uint256 _assets, Math.Rounding _rounding) internal view returns (uint256 _shares) {
        _shares = Math.mulDiv(
            _assets,
            uint256(getUnderlyingTknPriceStoredE18()),
    ...
    function deposit(uint256 _assetsIn, address _receiver) public returns (uint256 _sharesOut) {
        // Update the oracles if necessary
        updateVaultAndUnderlyingOracles();
    ...
    // Notice function to handle the update of both oracles
    function updateVaultAndUnderlyingOracles() internal {
        updateVaultTknOracle();
        updateUnderlyingTknOracle();
    }

    /// @notice Update the vault token price, if necessary
    function updateVaultTknOracle() public {
        // if the oracle was already read this block exit early, save gas
        if (block.number == lastVaultTknOracleRead) return ();
        ...
        lastVaultTknOracleRead = block.number;
    }

    /// @notice Update the vault token price, if necessary
    function updateUnderlyingTknOracle() public {
        /// if the oracle was already read this block exit early, save gas
        if (block.number == lastUnderlyingTknOracleRead) return ();

        // Read from the oracle
        uint256 fetchedPrice = uint256(getLatestUnderlyingPriceE18());

        // Set the new underlying token price
        underlyingTknPrice = fetchedPrice;

        // Note: this ignores updatedAt that is returned by the oracle
        lastUnderlyingTknOracleRead = block.number;
    }
    ...
    function setOracles(address _underlyingOracleAddr, address _vaultOracleAddr) public onlyOwner {
        ...
        if (_underlyingOracleAddr != address(0) && priceFeedUnderlying.decimals() != 18) {
            revert FeedsMustBe18Decimals();
        }
    ...
    /// @notice If the underlying token is not 18 decimals
    error UnderlyingTokenMustBe18Decimals();

    /// @notice If both of the oracle feeds are not 19 decimals
    error FeedsMustBe18Decimals();
}
